...
int rgb_keyboard::keyboard::write_brightness() {
	...
	std::copy(std::begin(_data_settings), std::end(_data_settings),
			  std::begin(data_settings));
...
int rgb_keyboard::keyboard::write_mode() {
	...
	// The magic numbers could be moved to data.cpp
	if (_profile == 1) {
		switch (_mode[_profile - 1]) {
		case m_horizontal_wave:
		...
		case m_rain:
			...
			break;
		case m_diagonal_wave:
			data_settings[1] = 0x17;
			data_settings[8] = 0x10;
			break;
		case m_reactive_color:
...
int rgb_keyboard::keyboard::write_color() {
	...
	std::copy(std::begin(_data_settings), std::end(_data_settings),
			  std::begin(data_settings_1));
	uint8_t data_settings_2[64];
	std::copy(std::begin(_data_settings), std::end(_data_settings),
			  std::begin(data_settings_2));
...
int rgb_keyboard::keyboard::write_custom() {
	...
	for (std::pair<std::string, std::array<uint8_t, 3>> element :
		 _key_colors[_profile - 1]) {
...
int rgb_keyboard::keyboard::write_active_profile() {
	...
	std::copy(std::begin(_data_profile), std::end(_data_profile),
			  std::begin(data_profile));
