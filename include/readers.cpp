...
int rgb_keyboard::keyboard::read_active_profile() {
	...
	std::copy(std::begin(_data_read), std::end(_data_read),
			  std::begin(data_read));
	data_read[1] = 0x2f;
	data_read[3] = 0x03;
	data_read[4] = 0x2c;
	...
	res += libusb_interrupt_transfer(_handle, 0x03, data_read, 64, &transferred,
									 1000);

	// read from endpoint 2
	res += libusb_interrupt_transfer(_handle, 0x82, buffer, 64, &transferred,
									 1000);
...
int rgb_keyboard::keyboard::read_led_settings() {
	...
	std::copy(std::begin(_data_read), std::end(_data_read),
			  std::begin(data_read_1));
	std::copy(std::begin(_data_read), std::end(_data_read),
			  std::begin(data_read_2));
	std::copy(std::begin(_data_read), std::end(_data_read),
			  std::begin(data_read_3));
	...
	res += libusb_interrupt_transfer(_handle, 0x03, data_read_1, 64,
									 &transferred, 1000);
	...
	res += libusb_interrupt_transfer(_handle, 0x03, data_read_2, 64,
									 &transferred, 1000);
	...
	res += libusb_interrupt_transfer(_handle, 0x03, data_read_3, 64,
									 &transferred, 1000);
	...
	for (int i = 0; i < 3; i++) {
		...
		// brightness
		if (input_buffer[i][9] >= _brightness_min &&
			input_buffer[i][9] <= _brightness_max)
			_brightness[i] = input_buffer[i][9];

		// speed
		if (3 - input_buffer[i][10] >= _speed_min &&
			3 - input_buffer[i][10] <= _speed_max)
		...
		case 0x06:
			...
			break;
		case 0x07:
			_mode[i] = m_reactive_single;
			break;
		case 0x08:
			_mode[i] = m_reactive_ripple;
			break;
		case 0x09:
			_mode[i] = m_reactive_horizontal;
			break;
		case 0x0a:
			_mode[i] = m_waterfall;
			break;
		...
		case 0x0f:
			...
			break;
		case 0x10:
			_mode[i] = m_diagonal_wave;
			break;
		case 0x11:
			_mode[i] = m_reactive_color;
			break;
		case 0x12:
			_mode[i] = m_ripple;
			break;
		...
		if (_mode[i] == m_reactive_color) {
			...
			case 0x00:
				_variant[i] = v_color_red;
				break;
			case 0x01:
				_variant[i] = v_color_yellow;
				break;
			case 0x02:
				_variant[i] = v_color_green;
				break;
			case 0x03:
				_variant[i] = v_color_blue;
				break;
			default:
				_variant[i] = v_undefined;
				break;
			}
		}
