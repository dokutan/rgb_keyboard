	...
	while (std::getline(config_in, line)) {
		...
		// is section a macro definition ?
			...
			if (std::regex_match(line, std::regex("repeat=[0-9]+"))) {
				uint8_t repeats = std::stoi(
					std::regex_replace(line, std::regex("repeat="), ""));
			...
			if (!std::regex_match(
					line, std::regex("[[:print:]]+=[[:print:]]+=[[:print:]]+")))
				continue;

			// extract individual parts of the macro command
			std::string type_string = std::regex_replace(
				line, std::regex("=[[:print:]]+=[[:print:]]+"), "");
			...
			int delay = std::stoi(std::regex_replace(
				line, std::regex("[[:print:]]+=[[:print:]]+="), ""));
