...
const std::map<uint16_t, bool> use_control_transfer = {
	...
	{0x5004, true}, {0x8520, true}, {0x652f, false}};

/// \return the VID of the first detected supported keyboard or 0 if no keyboard
/// was found
uint16_t detect_pid(uint16_t vid = keyboard_vid);
} // namespace rgb_keyboard

/**
 * This class represents the keyboard.
 *
 * The keyboard has 3 profiles with independent settings, all settings are
 * stored on the keyboard. To change the settings on the keyboard, the general
 * procedure is as follows:
 *
 * 1. Open the keyboard with open_keyboard() or open_keyboard_bus_device()
 * 2. Call set_*() or load_*() to change the private variables of this class.
 * The setter functions set the setting for \_profile (=1 by default), therefore
 * to change the settings for a different profile call set_profile() first.
...
class rgb_keyboard::keyboard {
  ...
  public:
	...
	enum report_rate { r_125Hz, r_250Hz, r_500Hz, r_1000Hz };
	 ...
	 * This changes the way data is send, the USB PID, the maximum brightness,
	 * etc.
	 */
	...
	/// Get currently set USB VID
	uint16_t get_vid();
	/// Get currently set USB PID
	uint16_t get_pid();
  ...
  private:
	...
	/** If this is variable is set to true, usb control transfers are used for
	 * sending data. This enables compatibility with other keyboards (Ajazz AK
	 * 33).
	 */
	bool _ajazzak33_compatibility = false;

	/// Profile (1-3): this determines the profile to which the settings are
	/// applied
	int _profile;
	...
	/// Maximum value for brightness (this is not constant, the Ajazz AK33 has a
	/// different value)
