...
int rgb_keyboard::keyboard::open_keyboard() {
	...
	// open device
	_handle =
		libusb_open_device_with_vid_pid(NULL, _keyboard_vid, _keyboard_pid);
...
int rgb_keyboard::keyboard::open_keyboard_bus_device(uint8_t bus,
													 uint8_t device) {
	...
	ssize_t num_devs =
		libusb_get_device_list(NULL, &dev_list); // get device list
...
int rgb_keyboard::keyboard::_write_data(unsigned char *data, int length) {
	...
	if (_ajazzak33_compatibility) {
		...
		result += libusb_control_transfer(_handle, 0x21, 0x09, 0x0204, 0x0001,
										  data, length, 1000);
		// read from endpoint 2
		result += libusb_interrupt_transfer(_handle, 0x82, buffer, 64,
											&transferred, 1000);

	} else {

		// write data packet to endpoint 3
		result += libusb_interrupt_transfer(_handle, 0x03, data, length,
											&transferred, 1000);
		// read from endpoint 2
		result += libusb_interrupt_transfer(_handle, 0x82, buffer, 64,
											&transferred, 1000);
